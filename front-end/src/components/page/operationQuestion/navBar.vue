...
<div>
  <div class="question_mount">
    ...
    <div class="button_action">
      ...
      <a href="javascript:;" class="question_view button_modal button_box" @click="answer_view" v-if="lockstate!=false">查看答案</a>
      <a href="javascript:;" class="question_active button_modal button_box" @click="answer_active" v-if="lockstate==false">激活答案</a>
    </div>
  </div>
    <el-dialog
    ...
    <span slot="footer" class="dialog-footer">
      ...
      <el-button type="primary" @click="view_answer">确 认</el-button>
    </span>
  </el-dialog>
<!--<el-steps>
...
<script>
    ...
    import {unlock} from '../../../api/getData.js'
      ...
      methods: {
        ...
        answer_active () {
          if (!getCookie('token')) {
            ...
            location.href = loginUrl + '&state=opera_' + questionId
            return
          } else if (JSON.parse(getStore('userinfo'))) {
            this.dialogVisible = true
            if (JSON.parse(getStore('userinfo')).ticketNum > 0) {
              this.discountInfo = '您有三张激活券，确认要激活该套题目的详细解析么？'
            } else {
        ...
        answer_view () {
          this.$router.push('/operationAnswer?id=' + this.$route.query.id)
        },
        view_answer () {
          if (JSON.parse(getStore('userinfo')).ticketNum > 0) {
            var data = {}
